...
def msg_decoder(msg_packet):
        message1 = msg_packet.decode("utf-8")
        #msg_type is data/start/end 
        msg_type, seq_num, recv_msg, check_sum = parse_packet(message1)
        return msg_type, seq_num, recv_msg, check_sum 
    
def msg_sender(sock, client_address, seq_num):
    msg_send = make_message('ack', 2,)
    msg_send = make_packet('ack', int(seq_num), msg_send)
    # print("ack sent", msg_send)
    sock.sendto(msg_send.encode("utf-8"), client_address)    
